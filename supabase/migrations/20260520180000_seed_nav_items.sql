-- Seed the header navigation with the items previously hardcoded in
-- src/components/Navigation.tsx. Idempotent: only runs if the header
-- has no rows yet, so re-running the migration won't duplicate items
-- or stomp admin edits.
DO $$
DECLARE
  products_id UUID;
BEGIN
